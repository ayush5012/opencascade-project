...
void occQt::makeBox()
{
        ...
        float y=400.0;
        float z=500.0;
        TopoDS_Shape aTopoBox = BRepPrimAPI_MakeBox(x,y,z).Shape();
        Handle(AIS_Shape) anAisBox = new AIS_Shape(aTopoBox);

        anAisBox->SetColor(Quantity_NOC_BLUE2);
...
void occQt::open()
{
      QString filename= QFileDialog::getOpenFileName(this,"Open A File","/home/ayush/");
}

void occQt::save()
{
      QString filename= QFileDialog::getSaveFileName(nullptr,"save a file","/home/ayush/");
}
void occQt::makeCone()
{
    gp_Ax2 anAxis;
...
void occQt::makeSphere()
{
    ...
    gp_Ax2 anAxis;
        ...
        TopoDS_Shape aTopoSphere = BRepPrimAPI_MakeSphere(anAxis, 300.0).Shape();
        Handle(AIS_Shape) anAisSphere = new AIS_Shape(aTopoSphere);

        anAisSphere->SetColor(Quantity_NOC_RED3);

        myOccView->getContext()->Display(anAisSphere, Standard_True);

}

TopoDS_Shape  occQt::ReturnBox()
{
    float x=500.0;
    float y=400.0;
    float z=500.0;
...
TopoDS_Shape occQt::ReturnSphere()
{
    gp_Ax2 anAxis;
    anAxis.SetLocation(gp_Pnt(0.0, 0.0, 0.0));
    //float radius=ui.Input1->text().toFloat();
    float radius =250.0;
    TopoDS_Shape aTopoSphere = BRepPrimAPI_MakeSphere(anAxis, radius).Shape();
    Handle(AIS_Shape) anAisSphere = new AIS_Shape(aTopoSphere);
...
void occQt::testCut()
{
        ...
        TopoDS_Shape aTopoBox = ReturnBox();
        TopoDS_Shape aTopoSphere = ReturnSphere();
        ...
        gp_Trsf aTrsf;
        aTrsf.SetTranslation(gp_Vec(-200.0, -600.0, 0.0));
        BRepBuilderAPI_Transform aTransform1(aCuttedShape1, aTrsf);

        aTrsf.SetTranslation(gp_Vec(400.0, 400.0, 0.0));
        ...
        anAisCuttedShape1->SetColor(Quantity_NOC_LIGHTSLATEBLUE);
        anAisCuttedShape2->SetColor(Quantity_NOC_THISTLE3);
...
{
        gp_Ax2 anAxis1;
        anAxis1.SetLocation(gp_Pnt(0.0, 0.0, 0.0));
        gp_Ax2 anAxis2;
        anAxis2.SetLocation(gp_Pnt(150.0, 150.0, 0.0));

        TopoDS_Shape aTopoBox = ReturnBox();          //BRepPrimAPI_MakeBox(anAxis1, 250.0, 250.0, 250.0).Shape();
        TopoDS_Shape aTopoSphere = ReturnSphere();     //BRepPrimAPI_MakeSphere(anAxis2, 200).Shape();
        TopoDS_Shape aCommonShape = BRepAlgoAPI_Common(aTopoBox, aTopoSphere);

        gp_Trsf aTrsf;
        aTrsf.SetTranslation(gp_Vec(-450.0, -450.0, 0.0));
        ...
        anAisBox->SetColor(Quantity_NOC_SPRINGGREEN);
        anAisSphere->SetColor(Quantity_NOC_STEELBLUE);
        anAisCommonShape->SetColor(Quantity_NOC_CORNSILK2);
